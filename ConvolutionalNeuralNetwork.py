...
class ConvolutionalNeuralNetwork:
	def __init__(self, input_depth=None, num_layers=2, num_filters=None, dropout_rate=0.5, regularization=0, verbose=True):
		self.graph = tf.Graph()
		with self.graph.as_default():
			...
			self.output = tf.reshape(dense2, [-1, self.board_size**2])
			self.softmax_output = tf.nn.softmax(self.output)			

			cond = tf.equal(tf.reduce_sum(self.y, axis=1), 1)
			self.y_pos = tf.where(cond, tf.abs(self.y), tf.zeros_like(self.y))
			self.y_neg = tf.where(cond, tf.zeros_like(self.y), tf.abs(self.y))
			self.loss = -tf.reduce_mean(self.y_pos*tf.log(self.softmax_output+1e-10) + self.y_neg*tf.log(1-self.softmax_output+1e-10))
			
			illegal_moves = 1 - tf.reshape(self.x[:,:,:,2], (-1, 64))
			self.loss += tf.reduce_sum(illegal_moves*self.softmax_output)

			reg = tf.contrib.layers.apply_regularization(tf.contrib.layers.l2_regularizer(float(regularization)), tf.trainable_variables())
			self.accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(self.output, axis = 1), tf.argmax(tf.abs(self.y), axis = 1)), tf.float32) )
			self.train_step = tf.train.AdamOptimizer(self.learning_rate).minimize(self.loss + reg)
